...
for x in range(1, 3):
    ...
    def __init__(self):
        print(UdemyParser.usage)
    
    def formatter(self, content:str) -> str:
    ...
    def parse_four_page(self, page_id:int) -> list[str]:
        soup_object = self.bs4_r(f"https://www.discudemy.com/all/{page_id}")

        queries = [self.formatter(query) for query in soup_object]
        if queries:
            courses=[]
            for q in queries:
                if self.find_url(q) != None:
                    if "couponCode" in self.find_url(curl=q):
                        courses.append(self.find_url(curl=q))
        return courses

    def data_export(self, courses:list, path="courses/", file="courses.txt") -> dict[str, int]:
